...
def index(request,pIndex=1):
    ...
    for vo in list2:
        vo.bank = Payment.objects.only("bank").get(order_id=vo.id).bank
        if vo.user_id == 0:
            vo.nickname = "无"
        else:
...
def insert(request):
    ...
    try:
        ...
        op.member_id = 0
        op.type = request.GET.get("ptype",1)
...
def status(request):
    ...
    try:
        ...
        payob = Payment.objects.get(order_id=oid)
        ob.status = request.GET['status']
        ob.payment_status = request.GET['payment_status']
        payob.status = request.GET['payment_status']
        ob.save()
        payob.save()
        return HttpResponse("Y")
    except Exception as err:
